...
const exportExcelData = async (semesterId) => {
  try {
    ...
    // Find semester data and populate the necessary fields (subject and teacher)
    const semester = await Semester.findById(semesterId).populate({
      path: "cac_lop_hoc_phan",
      populate: [
        { path: "subject" }, // Populate subject data
        { path: "giang_vien_phu_trach" } // Populate teacher without selecting specific fields
      ],
    });
    ...
    const data = [
      ...
      [
        ...
        "Sĩ số",
        "Loại MH",
        "Giảng viên phụ trách", // Added column for teacher
      ], // Header row
    ];

    for (const course of semester.cac_lop_hoc_phan) {
      const subject = course.subject;
      const teacher = course.giang_vien_phu_trach; // Teacher info
      ...
      data.push([
        ...
        course.si_so || 0,
        course.loai_mon_hoc || "Không xác định",
        teacher ? teacher.ten : "Không xác định", // Add teacher's name, default if not available
      ]);
    }
